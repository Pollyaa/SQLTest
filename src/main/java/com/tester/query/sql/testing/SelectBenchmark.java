...
import com.tester.query.sql.dao.SelectBenchmarkDao;
...
public class SelectBenchmark {
    ...
    private static final int TOTAL_QUERIES = 50000;
    private final Random random = new Random();

    public void runTesting() {
        WebSocketSessionManager.broadcast("{\"status\":\"running\",\"scenario\":\"Select Performance Test\"}");
        List<String> tables = new ArrayList<>();

        try (SelectBenchmarkDao dao = new SelectBenchmarkDao()) {
            WebSocketSessionManager.broadcast("{\"message\":\"\uD83C\uDFF3️ Створюються тестові таблиці для тесту\"}");

            for (int i = 0; i < TABLE_COUNT; i++) {
                String tableName = "select_test_table_%d_%d".formatted(System.currentTimeMillis(), i);
                boolean includeRelated = i > 0;
                dao.createTable(tableName, includeRelated);
                dao.insertRandomData(tableName, ROWS_PER_TABLE, DATA_LENGTH, includeRelated);
            ...
            while (totalQueriesExecuted < TOTAL_QUERIES) {
                ...
                for (int i = 0; i < queriesInBatch; i++) {
                    ...
                    try {
                        dao.selectRandomRow(table, ROWS_PER_TABLE);
                ...
                long batchDuration = (System.nanoTime() - batchStart) / 1000;
